    ...
    for position in self.beliefs:
    	if self.beliefs[position] > 0:
        	...
        	for x in self.legalPositions:
				newBeliefs[x] += self.beliefs[position]*newPosDist[x]
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
    	...
    	for position in self.legalPositions:
    		...
    		allPossible[position] += emissionModel[distance] * currentBelief[position]
...
class JointParticleFilter:
  ...
  def observeState(self, gameState):
    ...
    self.updateParticleWithGhostInJail(noisyDistances)

    for particle in self.particles:
      probability = 1.0
      for i, ghostPosition, model, noisyDistance in zip(range(len(particle)), particle, emissionModels, noisyDistances):
        if noisyDistance == None:
          if ghostPosition != self.getJailPosition(i): probability = 0
        else:
          probability *= model[util.manhattanDistance(pacmanPosition, ghostPosition)]
  ...
  def updateParticleWithGhostInJail(self, noisyDistances):
