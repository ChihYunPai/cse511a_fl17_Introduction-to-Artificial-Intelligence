...
class QLearningAgent(ReinforcementAgent):
  ...
  def getQValue(self, state, action):
    ...
    """
    "*** YOUR CODE HERE ***"
    if self.QValue.has_key(state) and len(self.QValue[state]) > 0:
      QValueList = self.QValue[state]
      if QValueList.has_key(action):
        return QValueList[action]
    return 0.0
  ...
  def getPolicy(self, state):
    ...
    legalActions = self.getLegalActions(state)
    bestAction = None
    if not legalActions: return bestAction
    actionValue = float('-inf')
    for action in legalActions:
      QValue = self.getQValue(state, action)
      if QValue > actionValue:
  ...
  def update(self, state, action, nextState, reward):
    ...
    """
    "*** YOUR CODE HERE ***"
    if not self.QValue.has_key(state):
      self.QValue[state] = {}
    if not self.QValue[state].has_key(action):
